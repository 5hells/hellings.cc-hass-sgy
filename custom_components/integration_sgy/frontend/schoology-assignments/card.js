class SchoologyAssignmentsCard extends HTMLElement {
  setConfig(config) {
    ...
    if (!config.entity.startsWith('sensor.') || !config.entity.includes('schoology') || !config.entity.includes('upcoming_assignments')) {
      throw new Error('Entity must be a Schoology upcoming assignments sensor');
    }
    this._config = config;
  ...
  set hass(hass) {
    ...
    const icon = this._config.icon || 'mdi:clipboard-text';
    if (!stateObj) {
